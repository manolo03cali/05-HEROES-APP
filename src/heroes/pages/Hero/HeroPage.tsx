// Importo componentes UI reutilizables que me ayudan a estructurar tarjetas, badges,
// progreso visual, tabs, etc.
...
// Importo mi acción personalizada para obtener un héroe desde la API.
import { getHeroAction } from "@/heroes/actions/get-hero.action";

// Uso React Query para manejar peticiones asincrónicas con caché y estados.
import { useQuery } from "@tanstack/react-query";

// Importo íconos de lucide-react para mejorar la interfaz visual.
import { Shield, Zap, Brain, Gauge, Users, Star, Award } from "lucide-react";

// useParams me permite leer el slug desde la URL; Navigate me permite redirigir.
import { Navigate, useParams } from "react-router";

// Componente principal de la página del héroe
export const HeroPage = () => {
  // Obtengo el idSlug desde la URL, por ejemplo "/heroes/batman"
  const { idSlug = "" } = useParams();

  // Hago la consulta del héroe usando React Query.
  // Configuro una clave de caché única por héroe.
  const { data: superheroData, isError } = useQuery({
    queryKey: ["heroes", idSlug],
    queryFn: () => getHeroAction(idSlug),
    retry: false, // Si falla la petición no reintento
  });

  // Si la API devuelve error, redirijo al usuario al home
  if (isError) {
    return <Navigate to="/" />;
  }

  // Mientras los datos aún no llegan, muestro un loading temporal
  if (!superheroData) {
    return <div>Loading...</div>;
  }

  // Calculo un indicador general del poder del héroe promediando sus stats
  const totalPower =
  ...
  const averagePower = Math.round((totalPower / 4) * 10); // porcentaje

  // Según el estado del héroe, devuelvo una clase CSS para colorear el badge
  ...
  // Según la categoría del héroe asigno un color especial
  ...
  // Renderizo la interfaz completa
  return (
    <div className="min-h-screen bg-gray-50">
      <CustomBreadcrums currentPage="Heroe" />

      {/* ENCABEZADO / BANNER */}
      <div className="bg-gradient-to-r from-slate-900 via-blue-900 to-slate-900 text-white">
        <div className="max-w-7xl mx-auto px-6 py-12">
          <div className="flex flex-col md:flex-row items-center gap-8">
            {/* Imagen principal del héroe */}
            ...
            {/* Información principal del héroe */}
            <div className="flex-1 text-center md:text-left">
              {/* Badges: categoría, estado, universo */}
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <Tabs defaultValue="stats" className="w-full">
          ...
          <TabsContent value="stats" className="space-y-6">
            {/* Cuatro tarjetas de estadísticas base */}
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {/* Renderizo cada poder desde el backend */}
          ...
          <TabsContent value="info">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* Información relacionada al universo del héroe */}
              <Card>
                <CardHeader>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex justify-between items-center py-2">
                    ...
                    {/* Calculo los años desde la primera aparición */}
                    <span className="font-semibold">
